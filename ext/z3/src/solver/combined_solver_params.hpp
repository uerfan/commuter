...
struct combined_solver_params {
  ...
  static void collect_param_descrs(param_descrs & d) {
    d.insert("solver2_timeout", CPK_UINT, "fallback to solver 1 after timeout even when in incremental model", "4294967295");
    d.insert("ignore_solver1", CPK_BOOL, "if true, solver 2 is always used", "false");
    d.insert("solver2_unknown", CPK_UINT, "what should be done when solver 2 returns unknown: 0 - just return unknown, 1 - execute solver 1 if quantifier free problem, 2 - execute solver 1", "1");
  }
  /*
