...
struct array_rewriter_params {
  ...
  static void collect_param_descrs(param_descrs & d) {
    d.insert("expand_select_store", CPK_BOOL, "replace a (select (store ...) ...) term by an if-then-else term", "false");
    d.insert("expand_store_eq", CPK_BOOL, "reduce (store ...) = (store ...) with a common base into selects", "false");
    d.insert("sort_store", CPK_BOOL, "sort nested stores when the indices are known to be different", "false");
  }
  /*
