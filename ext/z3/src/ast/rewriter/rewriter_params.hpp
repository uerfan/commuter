...
struct rewriter_params {
  ...
  static void collect_param_descrs(param_descrs & d) {
    ...
    d.insert("max_steps", CPK_UINT, "maximum number of steps", "4294967295");
    d.insert("flat", CPK_BOOL, "create nary applications for and,or,+,*,bvadd,bvmul,bvand,bvor,bvxor", "true");
    d.insert("push_ite_arith", CPK_BOOL, "push if-then-else over arithmetic terms.", "false");
    d.insert("push_ite_bv", CPK_BOOL, "push if-then-else over bit-vector terms.", "false");
    d.insert("pull_cheap_ite", CPK_BOOL, "pull if-then-else terms when cheap.", "false");
    d.insert("cache_all", CPK_BOOL, "cache all intermediate results.", "false");
  }
  /*
